...
namespace Unity.VectorGraphics
{
    public static partial class VectorUtils
    {
        ...
        /// <param name="svgPixelsPerUnit">How many SVG "pixels" map into a Unity unit</param>
        /// <param name="tr">A matrix transform to apply to the mesh</param>
        public static void FillMesh(Mesh mesh, List<Geometry> geoms, float svgPixelsPerUnit, Matrix4x4 tr)
        {
            ...
            if (tr == Matrix4x4.identity)
            {
                vertices3D = vertices.Select(v => (Vector3)v).ToList();
            }
            else
            {
                vertices3D = vertices.Select(v => tr.MultiplyPoint3x4(v)).ToList();
            }
        ...
        private static void FillVertexChannels(List<Geometry> geoms, float pixelsPerUnit, bool hasUVs, out List<Vector2> vertices, out List<UInt16> indices, out List<Color> colors, out List<Vector2> uvs, out List<Vector2> settingIndices, bool flipYAxis = false)
        {
